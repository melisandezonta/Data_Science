...
load_ts <- function(csv_filename){
    ...
    s = xts(coredata(df[,-1]),order.by = df[,1])
...
find_anomalies <- function(s, window_size=11, threshold=4) {
    ...
    # Hint: Look for data points that are more than (threshold * s.d.) away from mean
     ...
     anomalies = subset(s, abs(s[,1] - s.mean) > (threshold * s.sd))
     anomalies_index = which(s == anomalies)
     
    # TODO: Filter anomalies to only keep extrema
    # Hint: Look for peaks and troughs

     x = as.zoo(s)
     min_x <- rollapply(x,width = 10,FUN = min, align = "right",fill = "extend")
     max_x <- rollapply(x,width = 10,FUN = max, align = "right",fill = "extend")
     
     anomalies[which(min_x == min(s[anomalies_index]) || max_x == max(s[anomalies_index]))]
...
visualize <- function(res, wins=NA, title="Anomaly Detection Results") {
    ...
    lines(res$s.mean,col = 'blue') 
    
    # TODO: Draw margins at mean +/- (threshold * s.d.)
    
    res$s.margin.positive = res$s.mean + res$threshold * res$s.sd
    res$s.margin.negative = res$s.mean - res$threshold * res$s.sd
    
    lines(res$s.margin.positive, col = 'orange')
    lines(res$s.margin.negative, col = 'orange')

    # TODO: Mark anomalies
    
    points(x = res$anomalies, col = 'red')
}
